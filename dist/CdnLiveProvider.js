        ...
        for (const [idx, ch] of item.channels.entries()) {
          if (ch.url) {
            ...
            if (this.embedResolver) {
              ...
              const resolved = await this.embedResolver.resolve(ch.url, { referer: 'https://cdnlivetv.tv/', title: matchTitle }).catch(() => null);
              if (resolved) {
                streams.push(new StreamEntity({
                  name: `CDNLiveTV`,
                  title: ch.channel_name || `CDNLiveTV Stream ${idx + 1}`,
            ...
            streams.push(new StreamEntity({
              name: `CDNLiveTV`,
              title: ch.channel_name || `CDNLiveTV Stream ${idx + 1}`,
              externalUrl: `/watch?url=${encodeURIComponent(ch.url)}&title=${encodeURIComponent(matchTitle || 'CDNLiveTV')}`,
              resolution: 'HD'
            }));
          }
        }
